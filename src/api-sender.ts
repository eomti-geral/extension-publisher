...
async function updateOrCreateVariable(token: string, projectId: string, value: string, key: string): Promise<string> {
    ...
    try {
        ...
        if (error instanceof Error) {
            throw new TypeError(`Erro ao criar/atualizar variável ${key}: ${error.message}`);
        }
        throw error;
    }
}

export async function sendRefreshToken(token: string, value: string, key: string): Promise<string> {
