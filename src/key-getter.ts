...
if (typeof fetch !== 'function') {
  throw new TypeError(
    'This script requires Node.js 18.0 or newer because it relies on the global `fetch` function.'
  );
}
...
async function main() {
  ...
  async function writeToEnvFile(refreshToken: string): Promise<string> {
    ...
    // Remove todas as seções antigas de token
    const sections = content.split(
      '##############################################################################'
    );
    content = sections[0]; // Mantém apenas a primeira parte (variáveis originais)

    // Prepara a nova seção de token
    const newSection = [
      '##############################################################################',
    ...
    try {
      ...
      throw new Error(
        `Failed to write to .env file: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  const server = createServer(
    (request: IncomingMessage, response: ServerResponse) => {
      const searchParameters = new URL(request.url ?? '', `http://${localhost}`)
        .searchParams;
      if (searchParameters.has('code')) {
        approvalCode.resolve(searchParameters.get('code') ?? '');
        response.writeHead(200, { 'Content-Type': 'text/html' });
        response.end(
          'You can close this tab now. <script>window.close()</script>'
        );
        server.close();
        return;
      }
  ...
  async function getRefreshToken(): Promise<string> {
    ...
    if (!request.ok) {
      throw new Error(
        'Error while getting the refresh token: ' + request.statusText
      );
    }
    ...
    if (response.error) {
      throw new Error(
        'Error while getting the refresh token: ' + response.error
      );
    }
  ...
  p.intro('⚙️  Iniciando utilitário key-getter...');
  p.log.info(
    '🔑  Este utilitário irá gerar um refresh token para a API do Google Cloud. Ele será salvo no arquivo .env.'
  );
  if (isDev) {
    p.log.info('Executando em modo de desenvolvimento...');
    p.log.info(
      '🔑 O refresh token será salvo no arquivo .env. Você pode usar o comando `npm run dev` para gerar o bundle de desenvolvimento.'
    );
  }

  if (
    isDev &&
    envData.refreshToken &&
    envData.timestamp &&
    !hasOneDayPassed(envData.timestamp)
  ) {
    p.log.info('Checking last token request...');
    const lastRequestDate = new Date(envData.timestamp);
    p.outro(
      `❌ O último refresh token foi gerado em ${lastRequestDate.toLocaleString()} (menos de 24 horas).`
    );
    process.exit(0);
  }
  ...
  if (!clientId || !clientSecret) {
    console.error(
      'Error: GOOGLE_CLOUD_API_CLIENT_ID and GOOGLE_CLOUD_API_CLIENT_SECRET environment variables are required'
    );
  ...
  if (!gitLabToken) {
    console.error(
      'Error: GIT_LAB_ACCESS_TOKEN environment variable is required'
    );
  ...
  await tasks([
    {
      ...
      async task() {
        const instructions =
          'Complete the process in the browser. Follow its steps and warnings (this is your own personal app)';
        if (group.open) {
    ...
    },
    {
      ...
      async task() {
        return sendRefreshToken(
          gitLabToken,
          env.GOOGLE_CLOUD_API_CLIENT_ID!,
    ...
    },
    {
      ...
      async task() {
        return sendRefreshToken(
          gitLabToken,
          env.GOOGLE_CLOUD_API_CLIENT_SECRET!,
