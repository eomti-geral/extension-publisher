...
p.log.info('🔑  Este utilitário irá gerar um refresh token para a API do Google Cloud. Ele será salvo no arquivo .env.');
if (isDev) {
    p.log.info('Executando em modo de desenvolvimento...');
    p.log.info('🔑 O refresh token será salvo no arquivo .env. Você pode usar o comando `npm run dev` para gerar o bundle de desenvolvimento.');
}
...
if (isDev && envData.refreshToken && envData.timestamp && !hasOneDayPassed(envData.timestamp)) {
    ...
    p.outro(`❌ O último refresh token foi gerado em ${lastRequestDate.toLocaleString()} (menos de 24 horas).`);
    process.exit(0);
}
