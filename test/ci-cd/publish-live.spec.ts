...
test('A extensão deve estar em um estado que permita o envio e publicação de uma nova versão', 
    async ({ client, token }: TestContext) => {
        const myZipFile: fs.ReadStream = fs.createReadStream('./web-ext-artifacts/live-test.zip');

        console.log('🌐 Enviando versão de extensão de teste unitário');  
        const response: UploadResponse = await client.uploadExisting(myZipFile, token);
